...
const CLAVE_IDIOMA = "learnixIdioma";
...
function obtenerIdiomaInicial() {
    ...
    try {
        ...
        if (legacy.idioma === "es" || legacy.idioma === "en") return legacy.idioma;
    } catch {
...
export function LanguageProvider({ children }) {
    ...
    const cambiarIdioma = (nuevoIdioma) => {
        if (nuevoIdioma === "es" || nuevoIdioma === "en") setIdioma(nuevoIdioma);
    };

    const restablecerIdioma = () => setIdioma(IDIOMA_POR_DEFECTO);

    const t = (clave) =>
        traducciones[idioma]?.[clave] || traducciones.es?.[clave] || clave;

    return (
        <LanguageContext.Provider value={{ idioma, cambiarIdioma, restablecerIdioma, t }}>
            {children}
